...
- **ADR-020** (tomorrow is a day with a target too): `remainingPlan` spreads the gap over every day the month has left at one shared factor, and `Projection.TodayTarget`/`NextDayTarget` are two slices of it — so tomorrow's ask assumes today lands on today's. A weekday's historical average is what it *usually* takes, never what it is being asked for. `caixa.amanha` and `faturamento_de_hoje` exist for the same reason: a question about one day must not be answered with the month.
