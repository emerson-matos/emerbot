  ...
  it("nunca fala em vencimento, atraso ou inadimplência", () => {
    const frases = [30, 1, 0, 365]
      ...
      .concat(
        ...
        contaLegenda(
          makeDevedor({ saldo: 0, desde: null, dias_em_aberto: null }),
        ),
        contaLegenda(
          makeDevedor({ saldo: -500, desde: null, dias_em_aberto: null }),
        ),
...
describe("contaLegenda", () => {
  it("mostra a idade de quem está devendo", () => {
    expect(
      contaLegenda(makeDevedor({ saldo: 34000, dias_em_aberto: 60 })),
    ).toBe("em aberto há 60 dias");
  });
  ...
  it("não dá idade a quem está em crédito", () => {
    expect(
      contaLegenda(
        makeDevedor({ saldo: -5000, desde: null, dias_em_aberto: null }),
      ),
  ...
  it("não trava quando há dívida sem idade informada", () => {
    expect(
      contaLegenda(makeDevedor({ saldo: 1000, dias_em_aberto: null })),
    ).toBe("em aberto");
  });
});
...
describe("saldoTexto", () => {
  ...
  it("diz o crédito por extenso, com o valor no positivo", () => {
    expect(normalizeSpaces(saldoTexto(-5000))).toBe("R$ 50,00 em crédito");
  });
});
...
describe("formatação de datas e endereços", () => {
  ...
  it("escreve o dia por extenso, em minúsculas, para o render capitalizar", () => {
    expect(formatDiaLabel("2026-08-10")).toBe(
